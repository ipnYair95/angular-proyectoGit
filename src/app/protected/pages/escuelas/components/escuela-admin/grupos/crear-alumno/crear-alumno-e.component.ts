...
import { AlumnoService } from 'src/app/protected/pages/alumnos/services/alumno.service';
...
import { TutorEComponent } from './tutor/tutor.e.component';


@Component({
  selector: 'app-crear-alumno-e',
  templateUrl: './crear-alumno-e.component.html',
  styleUrls: ['./crear-alumno-e.component.css'],
})
export class CrearAlumnoEComponent implements OnInit {
  paginas = 0;
    ...
    this.es.obtenerEstados().subscribe((resp) => {
      ...
      if (id) {
        this.as.buscarPorId(id).subscribe(
          ...
          (err) => {
            if (err.status === 404) {
              Swal.fire('Error', `El alumno con id ${id} no existe`, 'error').then(resp => {
                this.router.navigate(['grupos'], {relativeTo: this.route});
                 }
              );
            }
  ...
  agregarTutor(indiceTutor?: number) {
    ...
    const modalRef = this.modalService.open(TutorEComponent, ngbModalOptions);
